...
import { FaArrowTrendUp, FaFire, FaStar } from 'react-icons/fa6'
...
export const Product: React.FC<ProductProps> = ({ title, colors, img, price, stars, status }) => {
  return (
    <Link to={''} className='relative flex flex-col justify-start w-[400px] z-[1] transition-all duration-[50ms] hover:drop-shadow-[2px_2px_10px]'>
      ...
      {status === 'popular' && (
        <Link to={''} className="absolute top-1 right-0 w-1/3 bg-zinc-800 flex items-center justify-center gap-2 py-2 rounded-2xl hover:scale-110 transition-transform">
          ...
          <p className='text-lg text-white font-semibold'>Popular</p>
        </Link>
      )}
      {status === 'onSale' && (
        <Link to={''} className="absolute top-1 right-0 w-1/3 bg-zinc-800 flex items-center justify-center gap-2 py-2 rounded-2xl hover:scale-110 transition-transform">
          <RiDiscountPercentFill className='text-red-500'/>
          <p className='text-lg text-white font-semibold'>On Sale</p>
        </Link>
      )}
